...
/* Fetching configuration from dts node */
#define ESF_NODE DT_NODELABEL(esp_loader0)
static const struct device *esf_uart = DEVICE_DT_GET(DT_PHANDLE(ESF_NODE, uart));
...
int install_image(const void *bin, size_t size, size_t offset);
...
static int cmd_esf_connect_rom(const struct shell *sh, size_t argc, char **argv)
{
    ...
    /* Hacky way how to deal with absence of pm_ API */
    uart_irq_tx_enable(esf_uart);
    uart_irq_rx_enable(esf_uart);

    loader_port_change_transmission_rate(esf_default_speed);
    k_msleep(5);

    if (esp_loader_connect(&esf_conn)) {
        shell_print(sh, "Failed to connect ROM");
        return -1;
    }
    shell_print(sh, "ROM connected at %d bps", esf_default_speed);
...
static int cmd_esf_connect_stub(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (esp_loader_connect_with_stub(&esf_conn)) {
        shell_print(sh, "Failed to connect STUB");
        return -1;
    }
    shell_print(sh, "STUB connected at %d", esf_default_speed);
...
static int cmd_esf_reset(const struct shell *sh, size_t argc, char **argv)
{
    ...
    /* Hacky way how to deal with absence of pm_ API */
    uart_irq_tx_disable(esf_uart);
    uart_irq_rx_disable(esf_uart);
...
static int cmd_esf_target_info(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (esp_loader_get_security_info(&info)) {
...
static int cmd_esf_flash_read(const struct shell *sh, size_t argc, char **argv)
{
    ...
    while (size > 0) {
        ...
        if (esp_loader_flash_read(buf, addr, to_read) != ESP_LOADER_SUCCESS) {
...
static int cmd_esf_flash_erase(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (argc > 1) {
        ...
        if (esp_loader_flash_erase_region(offset, size)) {
            shell_print(sh, "Failed to erase region");
            return -1;
        }
    } else {
        if (esp_loader_flash_erase()) {
...
static int cmd_esf_register_read(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (esp_loader_read_register(reg, &val)) {
...
static int cmd_esf_register_write(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (esp_loader_write_register(reg, val)) {
...
static int cmd_esf_speed_default(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (set_transmission_rate(esf_default_speed)) {
        shell_print(sh, "Failed to set transmission rate to %d", esf_default_speed);
    }
    return 0;
}
static int cmd_esf_speed_higher(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (set_transmission_rate(esf_higher_speed)) {
        shell_print(sh, "Failed to set transmission rate to %d", esf_higher_speed);
    }
    return 0;
}
