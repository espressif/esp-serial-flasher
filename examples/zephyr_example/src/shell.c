...
static int cmd_esf_reset(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (!connected) {
        shell_error(sh, "Target hasn't been connected");
        return -1;
    }
...
static int cmd_esf_target_info(const struct shell *sh, size_t argc, char **argv)
{
    ...
    if (!connected) {
        shell_error(sh, "Target not connected");
        return -1;
    }
    ...
    if (esp_loader_flash_detect_size(loader, &size)) {
        shell_error(sh, "Failed to detect flash size");
        return -1;
    }

    if (esp_loader_get_security_info(loader, &info)) {
        shell_error(sh, "Failed to read security information");
        return -1;
    }
...
static int cmd_esf_flash_read(const struct shell *sh, size_t argc, char **argv)
{
    ...
    while (size > 0) {
        ...
        if (esp_loader_flash_read(loader, buf, addr, to_read) != ESP_LOADER_SUCCESS) {
            shell_error(sh, "Failed to read flash");
            return -1;
        }
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_esf_flash,
                               SHELL_CMD_ARG(read, NULL, "<address> [<byte count>]", cmd_esf_flash_read, 2, 1),
                               SHELL_CMD_ARG(image, NULL, "[<img_index>], OR install all", cmd_esf_img_install, 1, 1),
                               SHELL_CMD_ARG(erase, NULL, "[<address> [<size>]]", cmd_esf_flash_erase, 1, 2),
                               SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_esf_register,
                               SHELL_CMD_ARG(read, NULL, "<register>", cmd_esf_register_read, 2, 0),
                               SHELL_CMD_ARG(write, NULL, "<register> [value]", cmd_esf_register_write, 3, 0),
                               SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_esf,
                               ...
                               SHELL_CMD(info, NULL, "[chip|flash|boot]", cmd_esf_target_info),
...
static const struct device *dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_esp_loader));
