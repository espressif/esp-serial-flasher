...
| `esp_loader_connect_secure_download_mode(&args, size, chip)` | `esp_loader_connect_secure_download_mode(&loader, &args, size)` — `chip` parameter removed; target is now always auto-detected via `GET_SECURITY_INFO`              |
