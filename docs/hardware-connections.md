...
|  BOOT  |  Host GPIO   |  Target IO0  | Boot mode selection (download mode)   |
...
**Important**: SDIO pins CMD and DAT0-3 **may require pullup resistors** depending on your hardware setup.
...
The SPI connection requires multiple pins for optimal performance:
...
**Strapping Pins**: Additional pins required for SPI download mode entry:

|   Pin    | Description     |
| :------: | :-------------- |
| STRAP_B0 | Strapping bit 0 |
| STRAP_B1 | Strapping bit 1 |
| STRAP_B2 | Strapping bit 2 |
| STRAP_B3 | Strapping bit 3 |

### Interface Notes

- **Strapping Pins**: Specific to each chip - consult target's Technical Reference Manual
- **IO MUX**: Pin selection affects maximum achievable speed
...
- **Host**: ESP32-S3, ESP32-S2 (with USB Host capability)
- **Target**: ESP32-S3 (with USB Serial/JTAG peripheral)
...
Choose the appropriate interface based on your requirements:

|    Interface    |   Speed    | Hardware Complexity | Target Support | Use Case                                |
| :-------------: | :--------: | :-----------------: | :------------: | :-------------------------------------- |
|    **UART**     | Low-Medium |       Simple        |   Universal    | General purpose, prototyping            |
|    **SDIO**     |    High    |       Medium        |    Limited     | High-speed programming (specific chips) |
|     **SPI**     |    High    |       Medium        |  Modern ESP32  | Fast RAM loading, development           |
| **USB CDC ACM** |   Medium   |       Simple        | ESP32-S3 only  | USB-only setups, convenience            |
