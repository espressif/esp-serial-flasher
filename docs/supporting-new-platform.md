...
- **Parameters**: `data` destination buffer; `size` requested bytes; `timeout` maximum time to wait [ms].
...
- **Parameters**: `function` SDIO function number; `addr` address within function; `data` destination buffer; `size` requested bytes; `timeout` maximum time to wait [ms].

- **Returns**: `ESP_LOADER_SUCCESS` on success, or `ESP_LOADER_ERROR_TIMEOUT`/other error codes.

- **Notes**: Should block until at least 1 byte is read or `timeout` elapses. Can be called repeatedly by higher layers until `size` is satisfied. Must be thread-safe for concurrent calls within the same thread.
...
- **Parameters**: `data` source buffer; `size` number of bytes to send; `timeout` timeout in milliseconds.
...
- **Parameters**: `function` SDIO function number; `addr` address within function; `data` source buffer; `size` number of bytes to send; `timeout` timeout in milliseconds.

- **Returns**: `ESP_LOADER_SUCCESS` on success, or error codes on failure.

- **Notes**: Should block until all bytes are queued/transmitted or an error occurs.
...
- **Parameters**: none.
- **Notes**: Typical UART sequence: assert BOOT (IO0) low, hold RESET low for `SERIAL_FLASHER_RESET_HOLD_TIME_MS`, keep BOOT asserted for `SERIAL_FLASHER_BOOT_HOLD_TIME_MS`, then release. Respect `SERIAL_FLASHER_RESET_INVERT` and `SERIAL_FLASHER_BOOT_INVERT` settings when controlling GPIOs.
...
- **Parameters**: `ms` delay time [ms].
- **Notes**: Used by the library for timing between steps.
...
- **Parameters**: `ms` timeout duration [ms].
- **Notes**: Subsequent calls to `loader_port_remaining_time()` should reflect remaining time relative to this start.
...
- **Notes**: Should never return a negative value; clamp to 0 when expired.
...
- **Parameters**: `level` boolean-like state (active/inactive).
- **Notes**: Ensure CS timing meets target requirements.
...
- **Notes**: Configure bus width, speed, and any interrupts required by the host SoC.
...
- **Notes**: Useful after initial sync to speed up transfers. Not available for SDIO interface.
...
- **Notes**: Can be used for recovery after errors.
...
- **Notes**: Route to your platform's logging/console facility.

## Implementation Steps

There are two main approaches to adding support for your platform:

### Option 1: Contributing to ESP Serial Flasher Repository
...
### Option 2: Using ESP Serial Flasher as External Library
