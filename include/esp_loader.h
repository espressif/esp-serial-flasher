 ...
 */
typedef enum {
    ESP_LOADER_CONNECT_MODE_RESET = 0, /*!< Enter bootloader before connecting. */
    ESP_LOADER_CONNECT_MODE_PASSIVE_FIRST, /*!< Connect without reset first, then reset and retry on failure. */
    ESP_LOADER_CONNECT_MODE_NO_RESET, /*!< Connect without reset and return the first connection result. */
...
#define ESP_LOADER_CONNECT_DEFAULT() { \
  .sync_timeout = 100, \
  .trials = 10, \
  .mode = ESP_LOADER_CONNECT_MODE_RESET, \
}
